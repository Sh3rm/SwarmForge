...
2. **Information Gathering (Parallel):** Read `.agents/model-list.txt` for the live model roster (maintained by the operator from an external terminal — never run `agy models` from the sandboxed terminal tool, it crashes; if the file is missing, ask the operator to regenerate it). Concurrently `invoke_subagent` the domain-relevant researchers (`researcher-tech-stack`, `researcher-google-cloud`, `researcher-security`, `researcher-vcs-github`, `researcher-academic-independent`, `researcher-anthropic-openai`) with the full Manifesto. If a local codebase exists, also spawn `repo-analyzer-worker`. Wait for all reports.
