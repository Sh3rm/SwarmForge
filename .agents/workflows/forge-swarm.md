...
2. **Information Gathering (Parallel):** Run `agy models` to capture the live model roster. Concurrently `invoke_subagent` the domain-relevant researchers (`researcher-tech-stack`, `researcher-google-cloud`, `researcher-security`, `researcher-vcs-github`, `researcher-academic-independent`, `researcher-anthropic-openai`) with the full Manifesto. If a local codebase exists, also spawn `repo-analyzer-worker`. Wait for all reports.
