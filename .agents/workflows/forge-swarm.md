...
2. **Information Gathering (Parallel):** Run `agy models < /dev/null` to capture the live model roster (the stdin redirect is mandatory — without it the command hangs; fall back to `> .agents/model-list.txt` + read if stdout is not captured). Concurrently `invoke_subagent` the domain-relevant researchers (`researcher-tech-stack`, `researcher-google-cloud`, `researcher-security`, `researcher-vcs-github`, `researcher-academic-independent`, `researcher-anthropic-openai`) with the full Manifesto. If a local codebase exists, also spawn `repo-analyzer-worker`. Wait for all reports.
