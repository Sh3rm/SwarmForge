...
2. **Design Blueprint & Benchmark-Driven Tiering:** Define the exact hierarchy of the new swarm. You MUST apply "Capability/Complexity Routing" grounded in live benchmark evidence — never in tier-name heuristics.
   ...
   - **Safe Default & Evidence Gate (CRITICAL):** Heavy-reasoning roles default to `model: inherit` (the session's model is typically the strongest available); assign `pro` ONLY with an explicit benchmark citation in the blueprint proving the current Pro family outperforms the session model. "Pro sounds stronger" is not evidence — it is the documented historical failure mode (the first forge run shipped `model: pro` on heavy roles and had to be repaired externally).
