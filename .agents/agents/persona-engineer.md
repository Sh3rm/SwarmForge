...
3. **Official Antigravity Schema (CRITICAL STRUCTURAL REQUIREMENT):**
   ...
   - Workers get `mainAgent: false` AND `subagent: true` — the two flags are mutually exclusive per file (`mainAgent: true` + `subagent: true` together is a contradiction; since v1.1.7 `subagent: false` agents cannot be invoked as subagents at all). Agents needing MCP servers get `inheritMcp: true` or a per-agent `mcpServers` list. Agents that run commands get `commandExecutionPolicy: sandbox`; agents that must not run commands get `off`; `auto`/`eager` NEVER on agents whose commands mutate system state.
