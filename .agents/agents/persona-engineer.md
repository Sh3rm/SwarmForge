...
3. **Official Antigravity Schema (CRITICAL STRUCTURAL REQUIREMENT):**
   ...
   - **No blueprint-field leakage:** Blueprint JSON fields (`tier`, `dependencies`, `tools_required`, `id`, `role`) are design-layer metadata — NEVER copy them into agent frontmatter as keys. The only model-related frontmatter key is `model:`; emitting a `tier:` key is a schema violation. Enum values must be exact: `commandExecutionPolicy` is `off|auto|eager|sandbox` (never invented hybrids like `auto-approve`).
